...
#20130217/{0}-20130217-pages-articles.xml.bz2
...
#print 'Will be parsing {0}'.format(path)


class DumpItem(pywikibot.ItemPage):
    def get(self, force=False, *args):
        ...
        if 'aliases' in self._content:
            for lang in self._content['aliases']:
                ...
                for value in self._content['aliases'][lang]:
                    self.aliases[lang].append(value)
        ...
        if 'claims' in self._content:
            for thing in self._content['claims']:
                claim = DumpClaim.fromJSON(self.repo, thing)
...
class DumpClaim(pywikibot.Claim):
    @staticmethod
    def fromJSON(site, data):
        ...
        in the API call.
        """
        claim = DumpClaim(site, 'p' + str(data['m'][1]))
        claim.snak = data['g']
        if data['refs']:
            for source in data['refs']:
                claim.sources.append(DumpClaim.referenceFromJSON(site, source))
        claim.isReference = False
        try:
            claim.type = data['m'][2]
        except IndexError:
            claim.target = None
        ...
        if claim.type == 'wikibase-entityid':
            claim.target = pywikibot.ItemPage(site, 'Q' +
                                          str(data['m'][3]['numeric-id']))
        else:
            claim.target = data['m'][3]
        return claim
    ...
    def referenceFromJSON(site, data):
        ...
        """
        data = data[0] #???
        claim = DumpClaim(site, 'p' + str(data[1]))
        claim.type = data[2]
        claim.isReference = True
        if claim.type == 'wikibase-entityid':
            claim.target = pywikibot.ItemPage(site, 'Q' +
                                              str(data[3]['numeric-id']))
        else:
            claim.target = data[3]


def DumpGenerator(repo=None):
    import json
    #do shit
    if not repo:
